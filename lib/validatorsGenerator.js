...
const errorsConf = require('./config/errorsConf');
...
class ValidatorsGenerator {
	...
	static createValidator(options) {
		let { errorsList, errorFieldsToGet } = options;
		...
		ValidationService.env = env;
		ValidationService.errorsList = this._mergeConfigAndUserErrors(errorsConf, errorsList);
		ValidationService.returnAllErrors = returnAllErrors;
		ValidationService.errorFieldsToGet = errorFieldsToGet;
